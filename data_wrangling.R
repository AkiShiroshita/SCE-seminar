...
df <- df %>% 
  ...
  select(id, age, sex, adm_adl, cap_hap, spo2, ams, sbp, immunodef, dev_place, adm_jcs, cci_score,
         ...
         intubation, oxy, emp_oxy, mecha_intu, base_dialysis,
  ...
  mutate(spo2 = if_else(dev_place == "8" | dev_place == "9" | spo2 == "9", NA_character_, spo2),
         ...
         dev_place = case_when(dev_place == "3" ~ "1",
                               dev_place == "5" ~ "0",
                               is.na(dev_place) ~ NA_character_)) %>% 
  mutate(sex = as.numeric(sex)) %>% 
  mutate(across(cap_hap:adm_jcs, ~ as.numeric(.x))) %>% 
